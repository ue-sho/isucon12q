  ...
  wrap(async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const player = await retrievePlayer(playerId)
        if (!player) {
          // 存在しないプレイヤー
      ...
      try {
        ...
        try {
          ...
          for (const player_id of Object.keys(playerScores)) {
            ...
            const p = await retrievePlayer(player_id)
            if (!p) {
              // 存在しない参加者が含まれている
...
app.get(
  ...
  wrap(async (req: Request, res: Response) => {
    try {
      ...
      const p = await retrievePlayer(playerId)
      if (!p) {
        throw new ErrorWithStatus(404, 'player not found')
      }
      const pd = {
      ...
      const [pss] = await adminDB.query<((PlayerScoreRow & CompetitionTitle) & RowDataPacket)[]>('SELECT player_score.*, competition.title FROM player_score JOIN competition ON competition.id = player_score.competition_id WHERE player_score.tenant_id = ? AND player_id = ?', [viewer.tenantId, pd.id])
      for (const ps of pss) {
        psds.push({
...
app.get(
  ...
  wrap(async (req: Request, res: Response) => {
    try {
      ...
      const p = await retrievePlayer(viewer.playerId)
      if (!p) {
        const data: MeResult = {
          tenant: td,
          me: null,
